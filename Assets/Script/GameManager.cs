...
public class GameManager : MonoBehaviour
{
        ...
        if(Input.GetMouseButton(0) || (Input.touchCount == 1 && Input.GetTouch(0).phase == TouchPhase.Moved))
        {
            ...
            if(wait)
            {
                ...
                if(move)
                {
                    ...
                    if (k == 0)
                    {
                        //가로 세로 같은 블럭 없으면 l이 0이 되어서 게임 오버
                        for (y = 0; y <= 3; y++)
                        {
                            for (x = 0; x <= 2; x++)
                            {
    ...
    void Spawn()
    {
        ...
        Square[x, y] = Instantiate(Random.Range(0, int.Parse(Score.text) > 800 ? 4 : 8) > 0 ? n[0] : n[1], new Vector3(-1.8f + 1.2f * x, -1.8f + 1.2f * y, 0), Quaternion.identity);
